...
- Path 노드 화면은 릴레이 상태만 표시 (`NORMAL` / `FAULT`) — BLE 링크를 겹쳐 쓰던 `WAIT` 제거
- 노드 주소를 기억해 스캔 없이 재접속하므로 한 노드를 잃어도 나머지 링크가 살아남음
- 노드 링크가 20초 이상 끊기면 컨트롤러가 자기 재부팅으로 복구 (RTC 카운터로 3회 제한)
